...
$('#send').on('click', function () {
    ...
    if (mensaje != '') {
        ...
        var fecha = Date.now();        
        $('#messages').append('<div class=\"outgoing_msg\"><div class=\"sent_msg\"><p>' + mensaje + '</p><span class=\"time_date\">' + dateFormat(fecha, " h:MM TT    |    mmmm dS") + '</span></div></div>');
        $('#myMessage').val('');
...
function PaintRecent(id, User, Client, Message, SocialNetwork, Time, userId) {
            ...
            function (response) {
                ...
                response.json().then(function (data) {
                    ...
                    for (let i = 0; i < data.length; i++) {                        
                        PaintRecent(data[i]._id, data[i].User, data[i].Client, data[i].Message, data[i].SocialNetwork, data[i].Time, userId);                        
                    }                    
                });
            }
        )
